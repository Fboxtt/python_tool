...
from language_manager import t, set_language, get_current_language  # 导入语言管理器
...
class BluetoothTool(QWidget):
    ...
    async def connect_serial_async(self):
        ...
        try:
            ...
            # 使用索引获取校验位（顺序：无/None, 奇校验/Odd, 偶校验/Even）
            parity_map = ['N', 'O', 'E']  # None, Odd, Even
            parity_index = self.parity_combo.currentIndex()
            parity = parity_map[parity_index] if 0 <= parity_index < len(parity_map) else 'N'
            ...
            # 校验位选择错误（不应该发生，但以防万一）
            self.blue_write_log(f"串口连接失败: 校验位配置错误 - {str(e)}")
        except ValueError as e:
            # 参数值错误
            self.blue_write_log(f"串口连接失败: 参数错误 - {str(e)}")
            ...
            self.blue_write_log(f"串口连接失败: {error_msg}")
            import traceback
            self.blue_write_log(f"详细错误: {traceback.format_exc()}")
    ...
    async def byte_send(self,data:bytes):
        ...
        try:
            if(not self.client or not self.client.is_connected):
                ...
                """蓝牙发送"""
            if self.client and self.client.is_connected:
                # 按实际MTU大小决定是否分包（不再依赖“分包”按钮）
                # 兼容不同Bleak版本：可能存在 client.mtu_size
                mtu_size = getattr(self.client, "mtu_size", None)
                mtu_payload = (mtu_size - 3) if isinstance(mtu_size, int) and mtu_size > 3 else None

                # 需求：如果是“128 MTU”就按128分包发送，否则直接整包发
                # 这里同时兼容两种说法：
                # - mtu_size == 128（把MTU当成payload的人）
                # - mtu_payload == 128（ATT MTU=131 → payload=128）
                should_split_128 = (mtu_size == 128) or (mtu_payload == 128)
                ...
                if should_split_128:
                    ...
                    for i in range(packet_count):
                        left = i * chunk_size
                        right = min(len(data), (i + 1) * chunk_size)
        ...
        try:
            ...
            while shake_count < 3:
                ...
                while err_count < 3:
                    ...
                    if not self.text_decode.have_hex:
                        await asyncio.sleep(time512 * 4)
                    if not self.text_decode.have_hex:
                        self.blue_write_log(f"❌ 握手超时，未收到回复 (第{err_count+1}次)")
                        err_count += 1
                    elif self.text_decode.is_download_cmd:
                        self.text_decode.reset()
            ...
            while err_count < 2:
                ...
                if not self.text_decode.have_hex:
                    await asyncio.sleep(time512 * 4)
                if not self.text_decode.have_hex:
                    self.blue_write_log(f"❌ 擦除超时，未收到回复 (第{err_count+1}次)")
                    err_count += 1
                ...
                if(data == None):
                    ...
                    while err_count < 5:
                        ...
                        if not self.text_decode.have_hex:
                            ...
                            err_count += 1
                        elif(self.text_decode.no80_cmd == BmsCmdType.WRITE_FLASH  and
                            self.text_decode.cmd_ack == 0x00 and
                            self.text_decode.cmd_packet_num == hex_packet + 1):
                                self.text_decode.reset()
                                if adaptive_wait is None:
                            ...
                            self.blue_write_log(f"❌ 包{hex_packet + 1}回复异常 (第{err_count+1}次) {self._ota_rx_diag(BmsCmdType.WRITE_FLASH, hex_packet + 1)}")
            ...
            while err_count < 5:
                ...
                if not self.text_decode.have_hex:
                    await asyncio.sleep(time512 * 4)
                if not self.text_decode.have_hex:
                    self.blue_write_log(f"❌ 总校验和超时，未收到回复 (第{err_count+1}次)")
                    err_count += 1
                elif(self.text_decode.no80_cmd == BmsCmdType.REC_TOTAL_CHECKSUM  and self.text_decode.cmd_ack == 0x00):
            ...
            while err_count < 5:
                ...
                if not self.text_decode.have_hex:
                    await asyncio.sleep(time512 * 4)
                if not self.text_decode.have_hex:
                    self.blue_write_log(f"❌ 71指令超时，未收到回复 (第{err_count+1}次)")
                    err_count += 1
                elif(self.text_decode.no80_cmd == BmsCmdType.READ_IC_INF  and self.text_decode.cmd_ack == 0x00):
                    self.text_decode.reset()
